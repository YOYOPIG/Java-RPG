...
public class Level1 extends Level {
	...
	private void renderingCandles() {
		...
		for (int i = 32; i < 56; i += 2) {
			if (i == 40) {
				...
				continue; // don't rendering candle to make a room for player to go to the exit
	...
	private void renderingLitCandles() {
		...
		// right side
		for (int i = 12; i < 36; i += 2) {
			tiles[54 + i * width] = Tile.litCandle11.getID();
			tiles[55 + i * width] = Tile.litCandle12.getID();
			tiles[54 + (i + 1) * width] = Tile.litCandle21.getID();
			tiles[55 + (i + 1) * width] = Tile.litCandle22.getID();
			System.out.println("123");
		}

		// down side
		for (int i = 54; i > 32; i -= 2) {
			tiles[i + 34 * width] = Tile.litCandle11.getID();
			tiles[i + 1 + 34 * width] = Tile.litCandle12.getID();
			tiles[i + 35 * width] = Tile.litCandle21.getID();
			tiles[i + 1 + 35 * width] = Tile.litCandle22.getID();
		...
		for (int i = 34; i > 12; i -= 2) {
			if (i == 32)
				continue;
			tiles[32 + i * width] = Tile.litCandle11.getID();
			tiles[33 + i * width] = Tile.litCandle12.getID();
			tiles[32 + (i + 1) * width] = Tile.litCandle21.getID();
			tiles[33 + (i + 1) * width] = Tile.litCandle22.getID();
		}
	}
	...
	public void renderingOpen(int position, int ID) {
		if (ID == 0 || ID == 1) { // npc and chest
			...
			// tiles[40+34*width] = Tile.litCandle11.getID();
			// tiles[41+34*width] = Tile.litCandle12.getID();
